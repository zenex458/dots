...
hypr_show(){
    ## taken from: https://github.com/hyprwm/Hyprland/discussions/830#discussioncomment-3868467
...
niri_show(){
    niri msg -j windows | jq '.[] | "\(.id) (\(.workspace_id)) \(.title) "' | bemenu -l 20 -i -p Open\ Windows: -P\> | awk -F ' ' '{print $1}' | sed 's/^"//' | xargs niri msg action focus-window --id
...
if [ "$XDG_CURRENT_DESKTOP" == "hyprland" ]; then
    ...
    [ "$XDG_CURRENT_DESKTOP" == "niri" ]; then
    niri_show
else
    printf "Unsupported desktop"
fi
