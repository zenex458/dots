...
Emacs does have some unintuitive keybinds, for example to search a file you press: =C-s=, which means holding down the control key and pressing the =s= key at the same time without releasing. To split your view you can press =C-x 2= for a horizontal split, and =C-x 3= for a vertical split. Thease require holding down the control key and pressing the =x= key at the same time, then leting go and pressing =2= or =3=. This is different to Neovim's [[https://en.wikipedia.org/wiki/Vi_(text_editor)#Interface][modal editing]] which is generally more efficient, and does not induce [[https://en.wikipedia.org/wiki/Emacs#Emacs_pinky][Emacs pinky]].
* Nixos
** Why Nixos?
*** Stable
